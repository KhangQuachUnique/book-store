...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (BufferedReader reader = req.getReader()) {
            ...
            if (email.isEmpty()) {
                JsonObject res = new JsonObject();
                res.addProperty("error", "Email cannot be empty!");
                resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.print(gson.toJson(res));
                return;
            }
            ...
            if (password.length() < 6) {
                JsonObject res = new JsonObject();
                res.addProperty("error", "Password must be at least 6 characters!");
                resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.print(gson.toJson(res));
                return;
            }

            // Create user object for JPA entity
            User user = new User();
            user.setName(name);
            user.setEmail(email.toLowerCase()); // Normalize email
            user.setPhoneNumber(phone); // Use correct field name for JPA entity

            // ⚡ Đăng ký + tạo verify token trong DB với JPA
            String verifyToken = userService.register(user, password);

            JsonObject res = new JsonObject();

            if (verifyToken == null) {
                // email exists or registration failed
            ...
            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            JsonObject res = new JsonObject();
            res.addProperty("error", "Validation error: " + cve.getMessage());
            out.print(gson.toJson(res));
        } catch (jakarta.persistence.PersistenceException pe) {
            // Handle JPA/Database errors (like duplicate email)
            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            JsonObject res = new JsonObject();
            if (pe.getCause() != null && pe.getCause().getMessage().contains("duplicate") || 
                pe.getMessage().contains("unique")) {
                res.addProperty("error", "Email already exists!");
            } else {
                res.addProperty("error", "Database error occurred. Please try again.");
            }
            out.print(gson.toJson(res));
        } catch (Exception e) {
            System.err.println("RegisterServlet: Unexpected error during registration:");
            e.printStackTrace();
            resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            JsonObject res = new JsonObject();
