...
public class JwtUtil {
    ...
    private static String generateToken(String email, String type, long expMillis, Map<String, String> claims) {
        ...
        if (claims != null) claims.forEach(builder::withClaim);
    ...
    public static boolean validateToken(String token) {
        try { decode(token); return true; }
        catch (Exception e) { return false; }
    }

    public static String getEmail(String token) { return decode(token).getSubject(); }

    public static String getRole(String token) { return decode(token).getClaim("role").asString(); }

    public static boolean isRefreshToken(String token) {
        try { return "refresh".equals(decode(token).getClaim("type").asString()); }
        catch (Exception e) { return false; }
    }
}
